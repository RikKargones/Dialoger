...
func _ready():
	...
	for item in Defaluts.LocalesNames.keys():
		...
		new_item.connect("local_toggled", self, "selection_changed")
		new_item.connect("font_changed", self, "selection_font_changed")
...
func selection_changed(element : LocaleElement, to : bool):
...
func selection_font_changed(element : LocaleElement, new_font : String):
